...
function BoothListPage() {
  ...
  const renderBoothItem = (booth, index) => {
    // 클릭 핸들러: 이미지가 있을 때만 상세 페이지로 이동
    const clickHandler = booth.image ? () => goToDetailPage(booth.id) : undefined;
    
    // CSS 클래스: 이미지가 있을 때만 클릭 가능하도록 커서 스타일 변경
    const clickClass = booth.image ? 'cursor-pointer hover:bg-white/10 transition-all duration-300 transform hover:-translate-y-1' : 'cursor-default';

    return (
      <div 
        key={booth.id} 
        // ✅ 수정: 이미지가 있을 때만 clickHandler 적용
        onClick={clickHandler} 
        className={`bg-white/5 backdrop-blur-xl rounded-2xl p-4 flex items-center gap-4 ${clickClass} animate-fade-in-up`}
  ...
  // renderDetailView 함수는 삭제했습니다. (단일 목록 페이지 구조로 변경)
